---
layout: distill
title: Parallelism for LLM Inferences
date: 2026-06-13
description: A note on parallelism strategies for large language model inference, including sharding, tensor parallelism, pipeline parallelism, and case studies.
categories: research
tags: LLM_Inference
...
toc:
  - name: Sharding Matrices
  - name: LLM Workloads
  - name: Shard on inputs
    subsections:
      - name: Data Parallel (DP)
      - name: Sequence Parallel (SP)
  - name: "Shard on weights: Tensor Parallel (TP)"
    subsections:
      - name: Sharding Attention
      - name: Expert Parallel (EP)
  - name: Pipeline Parallel (PP)
    subsections:
      - name: Sequence Pipeline Parallel (SPP)
  - name: Mixing Parallelism
  - name: "Case Study: llama3-70b"
---
...
<div class="row mt-3">
    <div class="col-lg mt-3 mt-md-0">
        {% include figure.liquid path="assets/notes/pipeline.jpg" class="img-fluid rounded z-depth-1" zoomable=true %}
    </div>
</div>
...
{% highlight python %}
{% include_relative scripts/llama3_modeling.py %}
{% endhighlight %}
